...
from cs231nimage_utils import SQUEEZENET_MEAN, SQUEEZENET_STD
...
def content_loss_test(correct, imgpath):
	...
	print('Maximum error is {:.3f}'.format(error), file=logfile)
...
def gram_matrix_test(correct, imgpath):
...
def style_loss_test(correct, imgpath):
	content_image = imgpath[0]
	style_image = imgpath[1]
	image_size = 192
	style_size = 192
	...
	error = rel_error(correct, student_output)
	logfile = open("./log.txt", 'a+')
	print('Error is {:.3f}'.format(error), file=logfile)
...
def tv_loss_test(correct, imgpath):
	content_image = imgpath[1]
	image_size = 192
	tv_weight = 2e-2
...
def style_transfer(content_image, style_image, image_size, style_size, content_layer, content_weight,
	...
	for t in range(500):
		...
		logfile = open("./log.txt", 'a+')
		if t % 50 == 0:
			print('Iteration {}'.format(t), file=logfile)
			plt.axis('off')
